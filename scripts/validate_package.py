...
REQUIRED = {
    ...
    "evals/failures.jsonl", "evals/config.json", "evals/run.py",
    "evals/graders.py",
}
...
def main() -> int:
    ...
    if eval_config_path.exists():
        ...
        if config.get("schema_version") != 2:
            fail("eval runner config schema_version must be 2", errors)
        expected_suites = {
            "routing": {"file": "routing.jsonl", "repetitions": 3,
                        "conditions": ["with_skill"]},
            "task": {"file": "tasks.jsonl", "repetitions": 5,
                     "conditions": ["with_skill", "without_skill"]},
            "failure": {"file": "failures.jsonl", "repetitions": 5,
                        "conditions": ["with_skill", "without_skill"]},
        }
        if config.get("suites") != expected_suites:
            fail("eval runner suite files, repetitions, or conditions are not canonical", errors)
        ...
        corpus_sizes = {
            name: len([line for line in (ROOT / "evals" / spec["file"]).read_text(encoding="utf-8").splitlines() if line.strip()])
            for name, spec in expected_suites.items()
        }
        planned = {
            name: corpus_sizes[name] * spec["repetitions"] * len(spec["conditions"])
            for name, spec in expected_suites.items()
        }
        if planned != {"routing": 150, "task": 100, "failure": 50}:
            fail(f"eval protocol must plan 150/100/50 trials, got {planned}", errors)

    graders_path = ROOT / "evals/graders.py"
    if graders_path.exists():
        grader_source = graders_path.read_text(encoding="utf-8")
        required_graders = {
            "behavioral_routing", "skill_selection", "neighbor_rejection", "live_boundary_routing",
