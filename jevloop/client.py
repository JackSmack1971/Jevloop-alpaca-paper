...
class MockDecisionClient(BaseDecisionClient):
    ...
    def _score(centre: float, labels: list[str]) -> dict:
        ...
        return {
            "type": "score",
            "score": round(score, 4),
            "legend": {str(i): v for i, v in enumerate(labels)},
    ...
    def ask(self, state: dict, questions: dict, timeout: float) -> tuple[dict, dict]:
        ...
        up = max(0.05, 0.34 + 0.25 * trend + 0.15 * imb)
        down = max(0.05, 0.34 - 0.25 * trend - 0.15 * imb)
        neutral = 0.32
        ...
        answers = {
            ...
            "quote_environment": self._score(
                max(0, min(3, 2.5 - 2 * liq)), ["Do not quote", "Marginal", "Standard", "Excellent"]
            ),
            "inventory_pressure": self._score(
                max(0, min(3, util * 3)), ["None", "Mild", "High", "Reduce now"]
            ),
    ...
    if direct_key:
        if not direct_model:
            raise DecisionClientError(
                "TYPESAFE_API_KEY is set but TYPESAFE_MODEL is empty; use GET /v1/models and pin one"
            )
        return TypeSafeDirectClient(direct_key, direct_model)
    if gateway_key:
        return GatewayClient(gateway_key)
    raise DecisionClientError(
        "no Jev provider key configured; use --mock only for offline/dry evaluation"
    )
