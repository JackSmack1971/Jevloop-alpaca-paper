...
- Requests has no default timeout; every external call here supplies one. Its connect/read timeouts are socket inactivity limits, **not** transport-level hard cancellation or a strict total wall-clock bound. The explicit `(connect, read)` tuples reduce long stalls, while the Jev client separately measures the complete operation with a monotonic clock (attempts, reads, retries, and backoff) and rejects returned payloads after the configured deadline. Evidence records that deadline, measured transport latency, and whether the response was accepted on time or rejected as late.
- **Architecture decision — strict interruption:** the current synchronous Requests transport cannot guarantee interruption at the exact deadline while an underlying call is still blocked. If strict wall-clock interruption becomes necessary, adopt a genuinely cancellable transport (or an isolated worker process that can be terminated) as a separate transport project, with cancellation, cleanup, connection reuse, and retry tests. Do not describe post-return deadline rejection as hard cancellation.
